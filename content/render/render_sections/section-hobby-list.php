...
<section class="dm-hobby-list">
    <container>
        ...
        <?php if( isset($jsonHobbyData["list"]) ): ?>
            <ul>
                <?php $i = 1; foreach ($jsonHobbyData["list"] as $item) : ?>

                    <li class="dm-hobby-item"

                        <?php if( isset($jsonHobbyData["img-path"]) && isset($item["img"]) ): ?>
                            style="background-image: url('<?php echo $GLOBALS['urlPath'] . "content/img/" . $jsonHobbyData["img-path"] . "/" . $item["img"]; ?>');"
                        <?php endif; ?>

                        data-motion="transition-fade-0" data-duration="0.4s" data-delay="<?php echo $i*0.05; ?>s">
